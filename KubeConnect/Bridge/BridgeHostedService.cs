...
namespace KubeConnect.Bridge
{
    public class BridgeHostedService : IHostedService
    {
        ...
        private static bool Match(V1ObjectMeta metadata, ServiceDetails service)
        {
        ...
        private static bool MatchTemplate(V1Deployment deployment, ServiceDetails service)
            => Match(deployment.Spec.Template.Metadata, service);

        private static bool Match(V1Pod pod, ServiceDetails service)
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var ns in namespaces)
            {
                ...
                {
                    var service = bridgingServices.Where(x => MatchTemplate(dep, x)).FirstOrDefault();
                    if (service != null)
                    {
                        var deployment = dep;
                ...
                foreach (var service in namespacedServices)
                {
                    ...
                    await kubernetes.CreateNamespacedPodAsync(pod, pod.Namespace());
